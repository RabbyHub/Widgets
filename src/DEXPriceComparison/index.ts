...
export default class DEXPriceComparison extends Widget {
  ...
  constructor(rule: string) {
    ...
    style.innerHTML = `
      .rabby-widget-dex-price-comparison, .rabby-widget-dex-price-comparison * {
        font-family: Arial, Helvetica, Tahoma, "PingFang SC", "Hiragino Sans GB", sans-serif;
      }
      .rabby-widget-dex-price-comparison {
        position: fixed;
      ...
      .rabby-widget-dex-price-comparison__content {
      ...
      .rabby-widget-dex-price-comparison__item .flex-1 {
        flex: 1;
        display: flex;
        align-items: center;
      }
      .rabby-widget-dex-price-comparison__item .icon-arrow {
        margin-left: 4px;
        margin-right: 4px;
        width: 12px;
      }
      .rabby-widget-dex-price-comparison__item .no-quote {
        font-size: 12px;
        line-height: 14px;
        color: #707280;
      }
      .rabby-widget-dex-price-comparison__popover {
        position: absolute;
        top: -30px;
        font-size: 12px;
        line-height: 16px;
      ...
      .rabby-widget-dex-price-comparison__item-amount {
      ...
      .rabby-widget-dex-price-comparison__item-symbol {
        display: inline-block;
        font-size: 12px;
        line-height: 14px;
        color: #13141A;
      ...
      .rabby-widget-dex-price-comparison__item-loss {
        font-size: 12px;
        line-height: 14px;
      }
      .rabby-widget-dex-price-comparison__item-loss.green {
        color: #00C087;
      }
      .rabby-widget-dex-price-comparison__item-loss.red {
        color: #EC5151;
      }
      .rabby-widget-dex-price-comparison__item.loading {
        background: linear-gradient(90deg, rgba(216, 223, 235, 0.55) 0%, rgba(216, 223, 235, 0.6) 95.31%);
        background-size: 400% 100%;
        -webkit-animation: rabby-widget-dex-price-comparison-loading 1.4s ease infinite;
        animation: rabby-widget-dex-price-comparison-loading 1.4s ease infinite;
      }
      .rabby-widget-dex-price-comparison__item:nth-last-child(1) {
        margin-bottom: 0;
      }
      @keyframes rabby-widget-dex-price-comparison-loading {
        0% {
  ...
  render = (values?: {
    ...
    if (this.el) {
      ...
      this.el = div;
      div.className = "rabby-widget-dex-price-comparison";
      div.innerHTML = `
        <div class="rabby-widget-dex-price-comparison__header">
          Prices elsewhere
        </div>
        <div class="rabby-widget-dex-price-comparison__content">
        </div>
      `;
    ...
    if (values) {
      ...
      values.quotes.forEach((quote) => {
        ...
        el.className = "rabby-widget-dex-price-comparison__item";
        if (quote.receive_token_amount) {
          ...
          imgWrapper.addEventListener("mouseenter", () => {
            ...
            popover.className = "rabby-widget-dex-price-comparison__popover";
            popover.innerText = quote.name;
            imgWrapper.appendChild(popover);
          });
          imgWrapper.addEventListener("mouseleave", () => {
            imgWrapper
              .querySelector(".rabby-widget-dex-price-comparison__popover")
              ?.remove();
          });
          el.innerHTML = `
            <div class="flex-1">
              <span class="rabby-widget-dex-price-comparison__item-amount" title="${new BigNumber(
                values.amount
              )
            ...
            .div(values.fromDecimals)
            .toFixed()}</span>
              <span class="rabby-widget-dex-price-comparison__item-symbol" title="${fromSymbol}">${fromSymbol}</span>
      ...
      Dexs.forEach(() => {
        const el = document.createElement("div");
        el.className = "rabby-widget-dex-price-comparison__item loading";
        content.appendChild(el);
      });
    }
  };
